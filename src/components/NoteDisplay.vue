...
export default {
  ...
  methods: {
    ...
    changeNote: function(event) {
      ...
      this.$emit("changeFreq", notes[index].freq);
    },
    handleKeyPress: function(event) {
        console.log("event.keyCode", event.keyCode, this.noteIndex, notes[this.noteIndex + 1].freq);
        // up 38, down 40
        if (event.keyCode == 38) {
            event.preventDefault();
            this.$emit("changeFreq", notes[this.noteIndex + 1].freq);
